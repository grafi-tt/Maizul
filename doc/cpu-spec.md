...
シリアル通信は，送受信とも3byteのバッファがあり，送信バッファが埋まっている，あるいは受信バッファに要素が無ければ，ブロックする（ノンブロッキングに使うこともできるが，互換性のため）．

命令RAMへの書き込み関連を追加．

#### get
この命令はもはや存在しない．getbと左シフトを使うこと．
...
標準出力なりシリアル通信なりから1byteのみ入力を受け取ってRyの最下位の値とする．Ryの上位はゼロクリアされる．
...
Rxの最下位1byteの値を標準出力なりシリアル通信なりに出力する．
...
# cget
Function `0000000000000110`

RxをR0に固定

シリアル通信の受信バッファに蓄えられている要素数を返す．0から3．

# cput
Function `0000000000000111`

RxをR0に固定

シリアル通信の送信バッファに蓄えられている要素数を返す．0から3．
