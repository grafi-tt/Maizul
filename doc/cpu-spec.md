...
要するに `01001000000xxxxx0000000000000000`で，xxxxxに入力された値を書き込むレジスタの番号を指定．
...
要するに `010010xxxxx000000000000000000001`で，xxxxxに出力する値を読み込むレジスタの番号を指定．

Rxの値を標準出力なりシリアル通信なりに出力する．単なるバイト列として，ビッグエンディアンなので最上位から順に出力を行う．コアで実行する際にはこの命令はブロックする（割り込みは未サポート）．

#### getb
Function `0000000000000010` / RS / type = 0

RxをR0に固定

要するに `01001000000xxxxx0000000000000010`で，xxxxxに入力された値を書き込むレジスタの番号を指定．

標準出力なりシリアル通信なりから1byteのみ入力を受け取ってRyの最下位の値とする．Ryの上位はゼロクリアされる．コアで実行する際にはこの命令はブロックする（割り込みは未サポート）．

#### putb
Function `0000000000000011` / RS / type = 0

RyをR0に固定

要するに `010010xxxxx000000000000000000011`で，xxxxxに出力する値を読み込むレジスタの番号を指定．
